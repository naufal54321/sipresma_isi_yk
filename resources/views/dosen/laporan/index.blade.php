    ...
    <div class="max-w-8xl mx-auto py-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
            ...
            <div class="flex gap-2 w-full md:w-auto">
                ...
                <button onclick="window.location.href='{{ route('dosen.laporan.export-excel', request()->query()) }}'"
                ...
                {{-- EXPORT PDF --}}
                <button onclick="window.location.href='{{ route('dosen.laporan.export-pdf', request()->query()) }}'"
        ...
        <div class="bg-white shadow-xl shadow-slate-200/40 rounded-2xl border border-slate-100 overflow-hidden">
            ...
            <div class="p-5 bg-slate-50/50 border-b border-slate-100">
                <form method="GET" action="{{ route('dosen.laporan.index') }}" class="flex flex-col md:flex-row items-center gap-3">
                    ...
                    {{-- ⚡ FILTER FAKULTAS --}}
                    <select name="fakultas" class="w-full md:w-48 border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer transition-shadow">
                ...
                <table class="w-full text-sm text-left text-slate-600">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-blue-50/50 transition duration-200">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $prodiData = \App\Models\ProgramStudi::where('nama_prodi', $item->user->prodi)->first();
                                @endphp
                                <span class="text-slate-700">{{ $prodiData->fakultas ?? '-' }}</span>
                            </td>
