            ...
            <form method="GET" action="{{ route('dosen.rpk.index') }}" class="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    @if(request('search') || request('tahun') || request('semester') || request('status'))
                        <a href="{{ route('dosen.rpk.index') }}" class="bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 px-5 py-2.5 rounded-xl text-sm font-semibold transition text-center flex items-center justify-center">
                            Reset
                        </a>
                    @endif
                </div>
