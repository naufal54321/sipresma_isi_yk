<nav x-data="{ collapsed: localStorage.getItem('sidebarState') === 'true', siapAnimasi: false }" 
     x-init="
        $watch('collapsed', val => localStorage.setItem('sidebarState', val));
        setTimeout(() => siapAnimasi = true, 100);
     "
    ...
    <div class="relative flex items-center py-5 border-b border-slate-800 bg-slate-900/50"
        ...
        <button @click="collapsed = !collapsed; $dispatch('sidebar-toggle', collapsed)" 
    ...
    @foreach(auth()->user()->roles as $role)
        @if($role->name == 'Admin')
            <span class="text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded-md">
                {{ $role->name }}
            </span>
        @elseif($role->name == 'Dosen')
            <span class="text-purple-400 bg-purple-500/10 border border-purple-500/20 px-2 py-0.5 rounded-md">
                {{ $role->name }}
            </span>
        @else
            <span class="text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-md">
                {{ $role->name }}
            </span>
        @endif
    @endforeach
    ...
    <div class="flex-1 min-h-0 overflow-y-auto overflow-x-hidden py-5 px-3 space-y-1.5 scrollbar-hide">
        ...
        <div x-data="{ open: {{ request()->routeIs('dosen.rpk.*', 'dosen.spk.*') ? 'true' : 'false' }} }">
            <button @click="if(collapsed) { collapsed = false; open = true; $dispatch('sidebar-toggle', false); } else { open = !open }" title="Validasi"
            ...
            <div x-show="open && !collapsed" 
                ...
                <a href="{{ route('dosen.rpk.index') }}"
                   class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transform transition-all duration-300 hover:translate-x-1 {{ request()->routeIs('dosen.rpk.*') ? 'text-blue-400 font-semibold bg-blue-500/10' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 shrink-0">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 0 0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5A3.375 3.375 0 0 0 6.375 7.5H5.25m11.9-3.664A2.251 2.251 0 0 0 15 2.25h-1.5a2.251 2.251 0 0 0-2.15 1.586m5.8 0c.065.21.1.433.1.664v.75h-6V4.5c0-.231.035-.454.1-.664M6.75 7.5H4.875c-.621 0-1.125.504-1.125 1.125v12c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V16.5a9 9 0 0 0-9-9Z" />
                    </svg>
                    <span>RPK</span>
                ...
                <a href="{{ route('dosen.spk.index') }}"
                   class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transform transition-all duration-300 hover:translate-x-1 {{ request()->routeIs('dosen.spk.*') ? 'text-blue-400 font-semibold bg-blue-500/10' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 shrink-0">
