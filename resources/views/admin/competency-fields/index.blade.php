<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-8xl mx-auto py-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">Bidang Kompetensi</h1>
                    <p class="text-gray-500 mt-1">Kelola data bidang kompetensi mahasiswa</p>
                </div>
            </div>

            <div class="bg-white shadow rounded-xl p-4 mb-4 flex flex-col md:flex-row items-center justify-between gap-4">
                <form method="GET" action="{{ route('admin.competency-fields.index') }}"
                    ...
                    <div class="relative w-full md:w-auto flex-shrink-0">
                        <input type="text"
                               ...
                               placeholder="Cari bidang kompetensi..."
                    ...
                    <div class="flex gap-2 w-full md:w-auto flex-shrink-0">
                        ...
                        @if(request('search'))
                            <a href="{{ route('admin.competency-fields.index') }}"
                    ...
                    <table class="w-full text-sm text-left text-gray-600">
                        <thead class="bg-gray-50 uppercase text-xs tracking-wider border-b-2 border-gray-200">
                            <tr>
                                ...
                                <th class="px-4 py-4 text-center">Jumlah Jenis</th>
                        ...
                        <tbody id="tableBody" class="divide-y divide-gray-200">
                            ...
                            <tr id="row-{{ $field->id }}" class="border-b hover:bg-blue-50 transition duration-150">
                                <td class="px-4 py-4 text-center">
                                    {{ ($fields->currentPage() - 1) * $fields->perPage() + $loop->iteration }}
                                </td>
                                <td class="px-4 py-4 font-medium text-gray-800">{{ e($field->name) }}</td>
                                <td class="px-4 py-4">{{ e($field->description ?? '-') }}</td>
                                <td class="px-4 py-4 text-center">
                                ...
                                <td class="px-4 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <button type="button"
                                                ...
                                                data-description="{{ e($field->description ?? '') }}"
                            ...
                            <tr id="emptyRow">
                                <td colspan="6" class="text-center py-10 text-gray-400">
                                    Belum ada data bidang kompetensi
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
        ...
        return `
            ...
            <div class="mb-4">
                ...
                <textarea id="form_description" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500"
                          placeholder="Deskripsi singkat bidang kompetensi...">${data.description || ''}</textarea>
            </div>
            <div class="mb-4">
    ...
    function validasiForm() {
        ...
        if (!name) { Swal.showValidationMessage('Nama bidang wajib diisi'); return false; }
        return true;
    }
    ...
    function bukaModalTambah() {
        Swal.fire({
            title: '<h2 class="text-xl font-bold text-gray-800 text-left">Tambah Bidang Kompetensi</h2>',
            width: '500px',
            ...
            if (result.isConfirmed) {
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Bidang kompetensi berhasil ditambahkan', timer: 2000, showConfirmButton: false })
    ...
    function bukaModalEdit(button) {
        ...
        Swal.fire({
            title: '<h2 class="text-xl font-bold text-gray-800 text-left">Edit Bidang Kompetensi</h2>',
            width: '500px',
            ...
            if (result.isConfirmed) {
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Bidang kompetensi berhasil diperbarui', timer: 2000, showConfirmButton: false })
                .then(() => location.reload());
            }
        });
    }
