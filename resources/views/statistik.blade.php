...
<head>
    ...
    <meta property="og:title" content="Statistik Prestasi — PRATAMA ISI Yogyakarta">
    <meta property="og:description" content="Lihat statistik dan data prestasi mahasiswa ISI Yogyakarta secara real-time.">
...
<nav class="fixed top-0 w-full z-50 bg-white/90 shadow-sm transition-all duration-300" id="navbar" aria-label="Navigasi Utama">
    <div class="flex justify-between items-center px-margin-mobile md:px-margin-desktop py-4 max-w-container-max mx-auto">
        ...
        <div class="hidden md:flex gap-6 items-center">
            ...
            <a class="nav-link text-primary font-bold border-b-2 border-primary pb-1 transition-colors hover:opacity-80" href="{{ url('/statistik') }}">Statistik</a>
            <a class="nav-link text-primary/80 hover:text-primary transition-colors" href="{{ url('/kontak') }}">Kontak</a>
        </div>
...
<div x-show="mobileOpen" class="fixed inset-0 z-40 bg-primary backdrop-blur-xl md:hidden flex flex-col items-center justify-center"
    ...
    <div class="flex flex-col items-center gap-8 text-white text-xl">
        ...
        <a @click="mobileOpen = false" href="{{ url('/statistik') }}" class="font-bold border-b-2 border-white pb-1">Statistik</a>
    ...
    <!-- Hero Stats -->
    <section class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop pb-10 flex-1 w-full" style="padding-top: 100px">

        <div class="text-center mb-8">
            <span class="inline-block py-1 px-3 rounded-full badge-tag font-semibold text-xs mb-3">Statistik</span>
            <h1 class="font-heading text-4xl font-bold text-primary">Data Prestasi</span></h1>
            <p class="text-on-surface-variant mt-2">Ringkasan data prestasi mahasiswa ISI Yogyakarta</p>
        </div>

        {{-- Stats Cards --}}
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
            @php
            $totalSpk = $spkDraft + $spkDisetujui;
            $berprestasiCount = $rekapPrestasi->count();
            $cards = [
                ['label' => 'Total Mahasiswa', 'value' => number_format($totalMahasiswa,0,',','.'), 'raw' => $totalMahasiswa, 'icon' => 'group'],
                ['label' => 'Total SPK', 'value' => number_format($totalSpk,0,',','.'), 'raw' => $totalSpk, 'icon' => 'emoji_events'],
                ['label' => 'SPK Disetujui', 'value' => number_format($spkDisetujui,0,',','.'), 'raw' => $spkDisetujui, 'icon' => 'check_circle'],
                ['label' => 'Berprestasi', 'value' => number_format(max($berprestasiCount, 0),0,',','.'), 'raw' => $berprestasiCount, 'icon' => 'workspace_premium'],
            ];
            @endphp
            @foreach($cards as $card)
            <div class="modern-card p-5 flex justify-between items-center group">
                <div class="min-w-0">
                    <p class="text-on-surface-variant text-[10px] font-bold uppercase tracking-widest">{{ $card['label'] }}</p>
                    <p class="text-3xl font-bold text-primary mt-1 counter-value" @if($card['raw']) data-value="{{ $card['raw'] }}" @endif>{{ $card['value'] }}</p>
                </div>
                <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform" 
                     style="background: linear-gradient(135deg, rgba(10, 25, 41, 0.15), rgba(30, 58, 138, 0.08)); color: var(--primary-dongker);">
                    <span class="material-symbols-outlined text-[28px]">{{ $card['icon'] }}</span>
                </div>
            </div>
            @endforeach
        </div>
        ...
        {{-- Chart Section --}}
        <div class="modern-card flex flex-col h-[300px] md:h-[450px] overflow-hidden mb-8">
            <div class="px-6 py-4 border-b border-outline-variant/30"><h3 class="font-heading font-bold text-primary">Statistik Prestasi Prodi ({{ date('Y') }})</h3></div>
            <div class="p-4 flex-1 relative"><canvas id="prestasiChart"></canvas></div>
        </div>

        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 mb-8">
            {{-- Rekap Terbaru dengan Status SPK --}}
            <div class="xl:col-span-4 modern-card flex flex-col h-[300px] md:h-[450px] overflow-hidden">
                <div class="px-6 py-4 border-b border-outline-variant/30 flex-shrink-0">
                    <h3 class="font-heading font-bold text-primary">Rekap Terbaru</h3>
                </div>
                <div class="flex-1 overflow-y-auto p-2 rekap-scroll">
                    @forelse($rekapPrestasi as $i => $spk)
                        <div class="p-3 hover:bg-slate-50 rounded-xl transition-colors border-b border-outline-variant/20 last:border-0 flex items-start gap-3">
                            <div class="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold shrink-0" 
                                 style="background: linear-gradient(135deg, rgba(10, 25, 41, 0.15), rgba(30, 58, 138, 0.08)); color: var(--primary-dongker);">
                                {{ strtoupper(substr($spk->user?->name ?? 'A', 0, 1)) }}
                            </div>
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center justify-between gap-2">
                                    <p class="text-primary font-bold text-sm truncate">
                                        {{ $spk->user?->name ?? 'Anonim' }}
                                    </p>
                                    @php
                                        $statusClass = match($spk->status ?? '') {
                                    ...
                                    @endphp
                                    <span class="text-[10px] px-2 py-0.5 rounded-full font-medium shrink-0 {{ $statusClass }}">
                                        {{ ucfirst($spk->status ?? 'Draft') }}
                                    </span>
                                </div>
                                <p class="text-xs text-on-surface-variant mt-0.5 line-clamp-1">
                                    {{ $spk->kegiatan?->kegiatan ?? $spk->keterangan ?? 'Tidak ada keterangan' }}
                                </p>
                            </div>
                        </div>
                    @empty
                        <div class="flex flex-col items-center justify-center h-full text-on-surface-variant text-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="modern-card flex flex-col h-[280px] md:h-[400px] overflow-hidden">
                <div class="px-6 py-4 border-b border-outline-variant/30 flex-shrink-0">
                    <h3 class="font-heading font-bold text-primary">Distribusi Jenis Kegiatan</h3>
                </div>
            ...
            <div class="modern-card flex flex-col h-[280px] md:h-[400px] overflow-hidden">
                <div class="px-6 py-4 border-b border-outline-variant/30 flex-shrink-0">
                    <h3 class="font-heading font-bold text-primary">Tren Bulanan ({{ date('Y') }})</h3>
                </div>
                <div class="p-6 flex-1 min-h-0">
        ...
        <div class="modern-card flex flex-col h-[250px] md:h-[350px] overflow-hidden">
            <div class="px-6 py-4 border-b border-outline-variant/30"><h3 class="font-heading font-bold text-primary">Top Penyelenggara</h3></div>
            <div class="p-4 flex-1 relative"><canvas id="penyelenggaraChart"></canvas></div>
        </div>
...
<footer class="bg-primary text-on-primary w-full mt-auto relative overflow-hidden border-t border-white/10" id="kontak">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ═══ PALET WARNA BIRU NAVY UNTUK CHART ═══
        const cp = ['#1E3A8A','#2563EB','#3B82F6','#60A5FA','#0A1929','#1E40AF','#1D4ED8','#93C5FD'];
        const tp = ['#1E3A8A','#2563EB','#3B82F6','#60A5FA','#0A1929','#1E40AF','#1D4ED8','#93C5FD'];
        
        Chart.defaults.font.family = "'Inter', sans-serif";
        Chart.defaults.color = '#44474e';
        const tt = { backgroundColor: 'rgba(10,25,41,0.95)', padding: 12, cornerRadius: 10 };

        // Chart 1: Statistik Prestasi Prodi
        new Chart(document.getElementById('prestasiChart'), {
            ...
            data: { 
                ...
                datasets: [{ 
                    data: @json($chartData), 
                    backgroundColor: '#1E3A8A', // Dongker
                    borderRadius: 8, 
                    barPercentage: 0.6 
                }] 
            },
        ...
        // Chart 2: Prestasi Berdasarkan Tingkat
        const tl = @json($tingkatLabels); 
        const td = @json($tingkatData);
        new Chart(document.getElementById('tingkatChart'), {
            ...
            data: { 
                ...
                datasets: [{ 
                    data: td, 
                    backgroundColor: tl.map((_,i) => tp[i%8]), 
                    borderRadius: 8, 
                    barPercentage: 0.5 
                }] 
            },
            options: { 
                ...
                plugins: { 
                    legend: { display: false }, 
                    tooltip: { ...tt, callbacks: { label: c => ` ${c.raw} Prestasi` } } 
                }, 
                scales: { 
                    x: { 
                        beginAtZero: true, 
                    ...
                    y: { 
                        grid: { display: false }, 
                        border: { display: false }, 
                        ticks: { font: { weight: '600' }, color: '#1E3A8A' } 
                    } 
                } 
            }
        });

        // Chart 3: Distribusi Jenis Kegiatan
        ...
        const tbl = @json($trenBulanLabels); 
        const tbd = @json($trenBulanData);
        ...
        // Chart 5: Top Penyelenggara
        const pyl = @json($penyelenggaraLabels); 
        const pyd = @json($penyelenggaraData);
        new Chart(document.getElementById('penyelenggaraChart'), {
            ...
            data: { 
                ...
                datasets: [{ 
                    data: pyd, 
                    backgroundColor: ['#0A1929','#1E3A8A','#2563EB','#3B82F6','#60A5FA'], // Gradasi navy
                    borderRadius: 6, 
                    barPercentage: 0.5 
                }] 
            },
            options: { 
                ...
                plugins: { 
                    legend: { display: false }, 
                    tooltip: { ...tt, callbacks: { label: c => ` ${c.raw} Kegiatan` } } 
                }, 
                scales: { 
                    x: { 
                        beginAtZero: true, 
