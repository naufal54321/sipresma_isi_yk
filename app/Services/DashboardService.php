...
class DashboardService
{
    ...
    public function getMahasiswaDosenKegiatan($userId)
    {
        ...
        return $kegiatans->groupBy(function ($kegiatan) {
                return $kegiatan->rpk?->dosenPembimbing?->id ?? 'tanpa-dosen';
            })
            ->map(function ($kegiatans, $dosenId) {
                if ($dosenId === 'tanpa-dosen') {
                    return null;
                }
                $dosen = $kegiatans->first()?->rpk?->dosenPembimbing;
                if (!$dosen) {
                    return null;
                }
                return [
